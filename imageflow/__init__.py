...
class ImageFlowWidget(QtWidgets.QWidget):
    ...
    def __init__(self, data=None, loadAsync=True, parent=None):
        ...
        if loadAsync:
            self.worker = Worker(self._o, self) # use the same dict, so worker always uses current options
            self.worker.start()
    ...
    def wheelEvent(self, event):
        if self.animator.target() is not None:
            delta = event.angleDelta().y() if hasattr(event, 'angleDelta') else event.delta() # Qt 5 vs 4
            self.showPosition(self.animator.target() - round(delta / 120))
        event.accept()
...
class Renderer:
    ...
    def __init__(self, widget):
        ...
        if widget.worker is not None:
            ...
            self.widget.worker.loadingStarted.connect(self._timer.start, Qt.QueuedConnection)
            self.widget.worker.loadingStopped.connect(self._timer.stop, Qt.QueuedConnection)
...
class Worker(QtCore.QThread):
    ...
    """
    loadingStarted = QtCore.pyqtSignal()
    loadingStopped = QtCore.pyqtSignal()
...
class ConfigWidget(QtWidgets.QWidget):
    ...
    def __init__(self, imageFlow, parent=None):
        ...
        sizeLabel = QtWidgets.QLabel(str(size))
        ...
        self.curveBox = QtWidgets.QComboBox()
        ...
        sliderLayout = QtWidgets.QHBoxLayout()
        numberSlider = QtWidgets.QSlider(Qt.Horizontal) 
        ...
        sizeLabel = QtWidgets.QLabel(str(imageFlow.option('imagesPerSide')))
        ...
        reflectionBox = QtWidgets.QCheckBox()
        ...
        fadeOutBox = QtWidgets.QCheckBox()
...
if __name__ == "__main__":
    ...
    configButton = QtWidgets.QPushButton("Options")
    ...
    aboutButton = QtWidgets.QPushButton("Info")
    def handleAboutButton():
        QtWidgets.QMessageBox.information(widget, "About Image Flow", "Image Flow by Martin Altmayer. Licensed under GPL v3. See https://github.com/MartinAltmayer/ImageFlow/")
